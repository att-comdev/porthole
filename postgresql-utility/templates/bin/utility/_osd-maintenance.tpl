...
function check_osd_status () {
  OSD_ID=$(nccli postgresql osd tree -f json-pretty | jq '.nodes[]|select(.type=="osd")|select(.status == "down")|.id')
  if [ "${OSD_ID}" != '' ];then
      for i in $OSD_ID; do
...
function osd_remove () {
  ...
  for id in $OSD_ID; do
   ...
   if [[ $YN == "y" || $YN == "Y" || $YN == "yes" || $YN == "YES" ]]; then
       echo "Purging OSD=$id"
       nccli postgresql osd purge $id --yes-i-really-mean-it
       sleep 3
...
function osd_remove_by_id () {
      OSDID=$1
      OSD_STATUS=$(nccli postgresql osd tree -f json-pretty | jq '.nodes[]|select(.type=="osd")|select(.id == '$OSDID')|.status')
      if [ "$OSD_STATUS" == '"down"' ]; then
          echo "OSD id $OSDID is in Down Status, So purging it"
          nccli postgresql osd purge $OSDID --yes-i-really-mean-it
...
function reweight_osds () {
  for OSD_ID in $(nccli postgresql osd df | awk '$3 == "0" {print $1}'); do
    OSD_WEIGHT=$(nccli postgresql osd df --format json-pretty| grep -A7 "\bosd.${OSD_ID}\b" | awk '/"kb"/{ gsub(",",""); d= $2/1073741824 ; r = sprintf("%.2f", d); print r }');
    nccli postgresql osd crush reweight osd.${OSD_ID} ${OSD_WEIGHT};
  done
}
