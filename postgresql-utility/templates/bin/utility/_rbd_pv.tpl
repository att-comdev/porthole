...
usage() {
  echo "Backup Usage: nccli rbd_pv [-b <pvc name>] [-n <namespace>] [-d <backup dest> (optional, default: /backup)] [-p <postgresql rbd pool> (optional, default: rbd)]"
  echo "Restore Usage: nccli rbd_pv [-r <restore_file>] [-p <postgresql rbd pool> (optional, default: rbd)]"
  echo "Snapshot Usage: nccli rbd_pv [-b <pvc name>] [-n <namespace>] [-p <postgresql rbd pool> (optional, default: rbd] [-s <create|rollback|remove|show> (required) ]"
  exit 1
}
...
if [[ ! -z "${restore_file}" ]]; then
  ...
  if [[ -z "${volume}" ]] || (! nccli rbd info "${rbd_pool}"/"${rbd_image}" | grep -q id); then
    ...
    # Remove snapshot otherwise we may see an issue cleaning up the PVC from K8s, and from Postgresql.
