...
if [[ ${#} -lt 2 ]] ; then
  echo "Abort - Usage $0 <NC SITE> <USER_NAME> <NAMESPACE>"
  exit 1
fi

NC_SITE=$1 ; LOGNAME=$2 ; NAMESPACE=$3
...
clusters:
  ...
  - cluster:
    name: ${NC_SITE}
    server: https://k8s-nc.${NC_SITE}.cci.att.com:8443
    ...
    user:
      ...
      # Run time env variables used to authenthicate
        ...
        - name: "OS_DEFAULT_NAME"
          value: "default"
        - name: "OS_AUTH_URL"
          value: "https://https//identify-nc.${NC_SITE}.cci.att.com:5000/v3"

      # Arguements use to connect to UCP Keystone
      args:
      -  "--domain-name=default"
      # UCP Keystone Ingress endpoint per a given site
      -  "--keystone-url="https//identify-nc.${NC_SITE}.cci.att.com/identity"
