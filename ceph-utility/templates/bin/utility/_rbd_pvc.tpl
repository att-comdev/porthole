...
usage() {
  echo "Backup Usage: $0 [-b <pvc name>] [-n <namespace>] [-d <backup dest> (optional, default: /tmp/backup)] [-p <ceph rbd pool> (optional, default: rbd)]"
...
if [[ ! -z "${restore_file}" ]]; then
  if [[ -e "${restore_file}" ]]; then
    ...
    if (nccli rbd info "${rbd_pool}"/"${rbd_image}" | grep -q id); then
      ...
      echo "WARNING: Existing RBD image has been moved to ${rbd_pool}/${rbd_image}.${timestamp}!" 
    fi
    unxz ${restore_file} -c | nccli rbd import - ${rbd_pool}/${rbd_image}
  ...
  if [[ -z "${volume}" ]] || (! nccli rbd info "${rbd_pool}"/"${rbd_image}" | grep -q id); then
    ...
    nccli rbd export ${rbd_pool}/${rbd_image}@${snap_name} - | xz  -0v --threads=0 > ${backup_dest}/${rbd_image}.img.xz
    # Remove snapshot otherwise we may see an issue cleaning up the PVC from K8s
    nccli rbd snap rm ${rbd_pool}/${rbd_image}@${snap_name}
    echo "PVC saved to ${backup_dest}/${rbd_image}.img.xz"
  fi
fi
