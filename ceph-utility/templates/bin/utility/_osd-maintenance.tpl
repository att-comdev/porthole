...
function osd_remove () {
  ...
  for id in $OSD_ID; do
   ...
   if [[ $YN == "y" || $YN == "Y" || $YN == "" || $YN == "yes" || $YN == "YES" ]]; then
       echo "Purging OSD=$id"
       nccli ceph osd purge $id --yes-i-really-mean-it
       sleep 3
   elif [[ $YN == "n" || $YN == "N" || $YN == "" || $YN == "no" || $YN == "NO" ]]; then
...
usage() {
  set +ex
  echo "Usage:  nccli osd-maintenance osd_remove"
  echo "        nccli osd-maintenance check_osd_status"
  echo "        nccli osd-maintenance reweight_osds"
  echo "        nccli osd-maintenance remove_osd_by_id --osd-id <OSDID>"
  exit 1
}
