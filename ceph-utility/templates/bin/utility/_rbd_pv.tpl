...
usage() {
  ...
  echo "Snapshot Usage: $0 [-b <pvc name>] [-n <namespace>] [-p <ceph rbd pool> (optional, default: rbd] [-s <create|rollback|remove> (required) ]"
  exit 1
}
...
if [[ ! -z "${restore_file}" ]]; then
  ...
  if [[ "x${snapshot}x" == "xcreatex" ]]; then
    ...
    echo "Snapshot ${rbd_pool}/${rbd_image}@${snap_name} has been created for ${pvc_name} with namespace ${nspace}"
    ...
    echo "WARNING: Rolled back snapshot ${rbd_pool}/${rbd_image}@${snap_name} for ${pvc_name} with namespace ${nspace}"
  elif [[ "x${snapshot}x" == "xremovex" ]]; then
    nccli rbd snap purge ${rbd_pool}/${rbd_image}
    echo "Removed snapshot(s) for ${pvc_name} with namespace ${nspace}"
  fi
else
