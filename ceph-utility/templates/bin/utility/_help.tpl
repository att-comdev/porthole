...
echo " nccli rados --pool <poolname> clonedata foo.tmp foo --object-locator foo"
echo " nccli rados --pool <poolname> bench 10 write -b 4000 -t 8 --run-name test"
echo " nccli rados --pool <poolname> cleanup"
echo "-------------"
echo " nccli radosgw-admin --help"
echo " nccli radosgw-admin user list"
echo " nccli radosgw-admin user info --uid=<id>"
echo " nccli radosgw-admin user stats --uid=<id>"
echo " nccli radosgw-admin bucket stats --bucket=<bucket>"
echo " nccli radosgw-admin bucket limit check --bucket=<bucket>"
echo " nccli radosgw-admin bucket list"
echo "-------------"
echo " nccli osd-maintenance"
echo "-------------"
echo " nccli rbd_pv"
echo "-------------"
echo " nccli rbd ls -l"
echo " nccli rbd ls -l {poolname}"
echo " nccli rbd snap ls {image-name}"
echo " nccli rbd --image {image-name} info"
echo " nccli rbd --pool {pool-name} snap ls {image-name}"
echo " nccli rbd device ls"
echo " nccli rbd help"
echo "-------------"
echo " nccli ceph {health|quorum_status|mon_status|status|df|fsid|--watch}"
echo " nccli ceph mon dump"
echo " nccli ceph auth list"
echo " nccli ceph pg dump"
echo " nccli ceph osd {stat|dump|tree}"
echo "-------------"
echo " nccli help"
echo "-------------"
  exit 1
